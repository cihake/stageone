...
// ─── Register a play (public) ─────────────────────────────────────────
// Client fires this after ~30s of continuous playback. Fire-and-forget; the
// client doesn't care about the response, only that the server got the ping.
trackRouter.post('/:trackId/play', async (req, res, next) => {
  try {
    const { slug, trackId } = req.params as { slug?: string; trackId?: string };
    if (!slug || !trackId) throw new HttpError(400, 'Missing slug or trackId');
    await incrementTrackPlayCount(slug, trackId);
    res.status(204).send();
  } catch (err) {
    next(err);
  }
