...
/**
 * Increment playCount for a published track. Public — no auth needed so
 * anonymous listeners still count. Client fires this after 30s of continuous
 * playback (mirroring Spotify's "counted play" threshold).
 *
 * We verify the track belongs to the artist for the slug so someone can't
 * spam-count arbitrary track IDs against the wrong artist.
 */
export async function incrementTrackPlayCount(slug: string, trackId: string): Promise<void> {
  if (!Types.ObjectId.isValid(trackId)) {
    throw new HttpError(400, 'Invalid track id');
  }
  const artist = await Artist.findOne({ slug: slug.toLowerCase(), isApproved: true });
  if (!artist) throw new HttpError(404, 'Artist not found');
  ...
  if (result.matchedCount === 0) throw new HttpError(404, 'Track not found');
}
